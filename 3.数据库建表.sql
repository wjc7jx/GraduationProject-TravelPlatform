...
CREATE TABLE `projects` (
  ...
  `review_status` ENUM('ok','pending','flagged') NOT NULL DEFAULT 'ok' COMMENT '内容合规：ok 已通过，pending 审核中，flagged 命中需修改',
  `review_reason` VARCHAR(64) DEFAULT NULL COMMENT '命中原因简述（label/suggest）',
  `review_checked_at` DATETIME DEFAULT NULL COMMENT '最近一次内容安全检测时间',
  `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  ...
  KEY `idx_projects_review_status` (`review_status`),
...
CREATE TABLE `contents` (
  ...
  KEY `idx_contents_project_review` (`project_id`, `review_status`),
